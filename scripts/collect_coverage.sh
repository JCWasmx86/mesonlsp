...
sudo cp build/meson_lsp /usr/bin
mkdir repos
mkdir /tmp/data
...
cd ..
build/meson_lsp repos/*/meson.build
mv default.profraw /tmp/data/repos.profraw
./build/meson_lsp --test TestCases/*/meson.build || exit
mv default.profraw /tmp/data/tests.profraw
build/meson_lsp --wrap Wraps/rustc-demangle.wrap --wrap Wraps/libswiftdemangle.wrap \
...
../../build/meson_lsp --wrap $(find . -maxdepth 1 -iname "*.wrap" | sed s/^.\\///g | paste -s | sed "s/\t/ --wrap /g") \
...
llvm-cov-17 export --instr-profile merged.profdata build/meson_lsp -format lcov >out.lcov
